...
userinit(void)
{
  ...
  kpt_copy(p->pagetable, p->kpagetable, 0, PGSIZE);
...
growproc(int n)
{
  ...
  sz = p->sz;
  if(n > 0){
    if (PGROUNDUP(n + sz) >= PLIC) return -1;
    if((sz = uvmalloc(p->pagetable, sz, sz + n)) == 0) {
      return -1;
    }
    kpt_copy(p->pagetable, p->kpagetable, sz - n, sz);
...
fork(void)
{
  ...
  kpt_copy(np->pagetable, np->kpagetable, 0, np->sz);
