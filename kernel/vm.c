...
void 
kpt_copy(pagetable_t src, pagetable_t dst, uint64 begin, uint64 end)
{
  for (int cur = PGROUNDUP(begin) ; cur < end; cur+=PGSIZE)
  {
    pte_t *s = walk(src, cur, 0), *d = walk(dst, cur, 1);
    if (s == 0 || d == 0) panic("kpt copy error");
    *d = (*s) & (~PTE_U);
  }
}